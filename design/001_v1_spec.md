...
- Landed notes (P5):
  ...
  - Parser is hand-rolled (no pyyaml): top-level single-line `key: value` between `---` fences, quotes stripped, nested keys ignored, CRLF + UTF-8 BOM tolerated, `utf-8` pinned. ~~YAML block-scalar markers (`>`, `|`, variants) as name/description raise instead of leaking `>` into the prompt.~~ *Revised post-P12: block scalars (`>`, `|`, chomping variants) now parse — `>`-family folds to spaces, `|`-family keeps newlines; only an unindented `---` closes the frontmatter. Prompted by skills.sh installs (`caveman` used `description: >` and bricked every agni turn).*
  - ~~Fail-loud by design: a malformed `SKILL.md` anywhere under the root (or a missing root) fails every agent's run/resume at entry, even agents filtered away from it. A skills root is config; a bad file should not be silently skipped.~~ *Revised post-P12: `~/.agents/skills` is shared with other tools, so a bad file must not brick the agent. `FileSystemSkills._scan` skips malformed/unreadable/duplicate skill dirs into `self.skipped: list[(path, reason)]` (refreshed per scan); a missing root still raises. agni surfaces skips: `MergedSkills.skipped` aggregate, stderr warning at startup, listed by `/skills`. `load()` of a skipped name still raises. Review round: 1 must-fix (non-UTF-8/unreadable files escaped the catch), 2 should-fix (refresh semantics unpinned, indented `---` inside a literal block truncated silently) — all applied, mutation-verified. 390 tests.*
...
### Phase 13 — Capability stress harness · deps: P2–P8 · **done**
Push the library past what the unit tests cover: 80% synthetic scenarios, 20% small live runs; every feature. `stress/` at the repo root is a committed, reusable pytest harness — not a workspace member, not in `testpaths` (`just stress` runs it, `just test` never collects it), imports **only public `tantra` names** so it doubles as an adopter/API-surface check.
- `stress/driver.py`: `SyntheticProvider` — policy-driven programmatic `Provider` (FakeProvider is order-scripted; marathons need policy), deterministic, small `ModelLimits` (32k/2k) to force compaction cheaply; policy helpers (`worker_policy`, `by_model`, `flaky`); `SyntheticEmbedder`. `stress/invariants.py`: `check_pairs` (no orphaned tool_call/result in any assembled request — the OpenAI-400 producer), `check_window` (every request ≤ usable×slack), `check_log` (seq contiguity, header/lease consistency), `pairs_intact` (log-level). `stress/conftest.py`: `store` fixture over all four backends (PG via the P8 docker auto-spin; per-test schemas dropped).
- Scenarios: `test_marathon.py` (60+ turns, repeated summarize+prune, marker-fact survival into every brief, suspend-after-compaction + fresh-harness resume, monster-turn prune-only, mid-marathon process swap), `test_tree.py` (max-depth bubble-ask with fresh-harness two-step resumes, fan_out 12 mixed outcomes, abandon-mid-spawn no-twin, derived permissions at depth), `test_kitchen.py` (raw-library adopter app: six hooks, permission matrix, ctx.ask re-execution semantics, memory incl. embed-failure/backfill/hybrid, skills incl. block-scalar + skipped, output_schema, cross-harness cancel, max_steps orphans, invalid JSON, retry recovery/exhaustion), `test_scale.py` (10k events, 200-session paging, 1k memory rows, cross-instance handoff, lease contention).
- Live slice (manual): `stress/live_raw.py` (raw harness on the GLM endpoint) + one agni `--auto` scratch task.
- **Verify:** `just stress` green on all four backends; `just lint` + `just test` untouched; harness defects found go through the standard fix round.
- Checklist:
  - [x] stress/ harness (driver + invariants + 4 scenario modules, all four backends)
  - [x] Review round (mutation-tested) + fix round
  - [x] Library defect fix round: compaction prune re-stub
  - [x] Live slice (live_raw + agni --auto)
- Landed notes (P13):
  - **Defect found and fixed — prune re-stub loop** (`compaction.py`). Stage-1 stubs append at the log tail, inside the protected `tail_turns`, but `_prune` built its last-wins `latest` map over the *prefix* only — so every compact re-stubbed the same call_ids, `gained` counted phantom reclaim vs the original content, and stage 2 was masked while the assembled request sat over `usable` (measured peak 1.30×; one call stubbed 4×, 285 stub events where 96 suffice) until old stubs scrolled into the prefix. Fix: build `latest` over the whole window (candidates still prefix-only; in-tail results never stubbed; stubs in the prefix fall out via `MIN_RESULT_CHARS`). After: exactly one stub per call, peak 1.09× usable, summarize fires on the same `compact()` call once the pool is dry. +2 unit tests (392 total); the stress xfails that pinned the defect flipped green at the default 1.25 estimator slack.
  - Suite: 82 passed / 2 skipped (MemoryStore has no shared substrate for handoff/lease-contention — honest skips), ~2 min with docker PG. Determinism verified across runs. SQLite is the slow backend (fsync per commit) — scale counts are reduced there.
  - Review round: 9 temporary library mutations — 8 caught by the harness; the 9th exposed that marker-survival was tautological (the synthetic summarizer echoed its own constant; it now echoes only markers actually present in the summarize request, and gutting the request history makes the marathon fail). 1 must-fix + 6 should-fix applied: invariants hardened against vacuous passes (empty request lists, shape drift, zero-pair walks all raise; checkers return counts callers pin), resume-path providers actually pair-checked in the tree suite, xfails made strict and re-emission-shaped.
  - API-surface findings (recorded, not changed): provider event types (`ToolCall`, `StreamEnd`, deltas) are not exported though any custom `Provider` — and even constructing the public `Sample` — needs them (the harness's one sanctioned non-public import); concrete session event classes (`TurnStarted`, `ToolCallCompleted`, …) unexported — consumers filter by the `type` discriminator string; message classes unexported; `Hook.before_tool`'s transform contract names the unexported `ToolCallRequested` (workaround: `model_copy`); `Store.close()` and pgvector's `memory_search` are duck-typed only.
  - Live slice, zero human input, all stderr empty: `live_raw.py` on `z-ai/glm-5.2` — lead delegated to a subagent (child events at depth 1), suspended on a real in-tool `ctx.ask` mid-turn, resumed from stdin, ended via `submit_output` with the parsed `Verdict` on `TurnCompleted.output`. agni `--auto` scratch task — explore delegate, block-scalar skill loaded and followed, two functions renamed with call sites, self-verified by running the module, `memory_write` recorded; a fresh session recalled the record via `memory_recall`.
  - Coverage gaps recorded, deliberate: hard process-death (live lease + `running` status) is mutation-verified but has no standing scenario (abandon-via-`aclosing` settles cleanly; P12 hit the real one live); cross-harness `cancel` interleaves at a store boundary rather than racing a concurrent consumer.

### Phase 14 — Terminal-Bench harness check · deps: P11, P12 · —
