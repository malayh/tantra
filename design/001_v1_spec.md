...
- **Hand-rolled SSE/accumulation over the official `openai` SDK** (decided post-P1, on review). The SDK would replace ~70 lines (SSE decode + fragment accumulation) but its typed models stop exactly where OpenRouter deviates (`reasoning` delta rides `model_extra` either way), we'd run it with `max_retries=0` and translate its exceptions since the loop owns retry, and the mapping layer — where most P1 review defects actually were — stays ours regardless. Cheap to reverse: confined to `openai_compat.py`, and the tests assert `ProviderEvent` sequences through an injectable transport, not internals.
