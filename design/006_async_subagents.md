...
- **Follow-up:** The lost post-Stop and idle-delivery defects are fixed with request-correlated durable acceptance, idempotent turn/message IDs, cancelling-turn rejection, multi-tab broadcasting, and retained UI drafts. A visible headed two-tab browser run verified live steering and child kill, one copy of root/descendant events in both tabs, one new turn after Stop, retained single delivery across a forced backend pause and socket reconnect, and stable task IDs, transcript, kill state, and message count across active refresh; both browser consoles were clean. The run also exposed and fixed an accepted first-message draft remaining in the composer. `agent-browser` could not attach to the existing Brave instance, so the run used its visible headed Chromium and the prior attachment/video checks remain open. Large recursive replay is buffered and applied atomically before readiness so historical replay is not rendered as live streaming. `EventBase` now recursively normalizes literal NUL characters to U+FFFD before durable persistence, unexpected Sarathi pump failures send one generic `server_error` and close with code 1011, and the UI does not reconnect on 1011. After rebuilding the Docker Compose backend and UI, the reported 1,750-event chat `8dd0...` stayed stable in Brave for 15 seconds across five former blink intervals: two connection opens total, zero closes or reopens during the observation window, zero pump, NUL, or 1011 errors, no browser console errors, and legitimate live events continued. Exact `just`/`uv` wrappers remain unavailable; focused Tantra tests have 73 passes, real PostgreSQL conformance has 18 passes, all 93 Sarathi backend tests pass, and UI tests have 9 passes with lint and build passing. Root lint and format pass; root tests have 705 passes and 15 skips, with only the deferred Agni blocking-delegate test failing.
